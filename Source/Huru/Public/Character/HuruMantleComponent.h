...
public:
	...
	bool MantleCheck(const FHuruMantleTraceSettings& TraceSettings,
					 EDrawDebugTrace::Type DebugType);

	UFUNCTION(BlueprintCallable, Category = "Huru|Mantle System")
	void MantleStart(float MantleHeight, const FHuruComponentAndTransform& MantleLedgeWS,
	...
	/** BP에서 캐릭터 상태에 따라 올바른 맨틀 파라미터 세트를 얻도록 구현함 */
...
protected:
	...
	void Server_MantleStart(float MantleHeight, const FHuruComponentAndTransform& MantleLedgeWS,
							EHuruMantleType MantleType);

	UFUNCTION(BlueprintCallable, NetMulticast, Reliable, Category = "Huru|Mantle System")
	void Multicast_MantleStart(float MantleHeight, const FHuruComponentAndTransform& MantleLedgeWS,
							   EHuruMantleType MantleType);
